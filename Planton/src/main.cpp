...
void drawFrame2(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->drawString(3 + x, 35 + y,"Lamp ON : " + String(stateOUT ? "ON": "OFF"));
}

void drawFrame3(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  display->setFont(ArialMT_Plain_10);
  display->drawString(3 + x, 12 + y, "Mon: " + String(days[1].isEnable) + "  " + days[1].startHours + " " +days[1].stopHours );
  display->drawString(3 + x, 22 + y, "Tue: " + String(days[2].isEnable) + "  " + days[2].startHours + " " +days[2].stopHours );
  display->drawString(3 + x, 32 + y, "Wed: " + String(days[3].isEnable) + "  " + days[3].startHours + " " +days[3].stopHours );
  display->drawString(3 + x, 42 + y, "Thu: " + String(days[4].isEnable) + "  " + days[4].startHours + " " +days[4].stopHours );
  display->drawString(3 + x, 52 + y, "Fri: " + String(days[5].isEnable) + "  " + days[5].startHours + " " +days[5].stopHours ); 
...
void drawFrame4(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) 
{
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  display->setFont(ArialMT_Plain_10);
  display->drawString(3 + x, 12 + y, "Sat: " + String(days[6].isEnable) + "  " + days[5].startHours + " "+days[6].stopHours ); 
  display->drawString(3 + x, 22 + y, "Sun: " + String(days[0].isEnable) + "  " + days[6].startHours + " "+days[0].stopHours );      
...
void setup(){
  ...
  if (WEBSERVER_ENABLED) {
    ...
    server.on("/readsystem", handleRead);
    server.on("/toggleLamp",handleToggle);
...
void loop(){
  ...
  if (remainingTimeBudget > 0) 
  {
    if (timeEpoch >= lastEpoch + refresh)
    {
      ...
      Serial.println("-----------");
      Serial.println("MAIN");
      Serial.println("stateOUT: " + String(stateOUT ? "ON": "OFF"));
      Serial.println("SOILhum: " + String(SOILhum));
      Serial.println("SoilRawValue: " + String(SoilRawValue));
      tick();
